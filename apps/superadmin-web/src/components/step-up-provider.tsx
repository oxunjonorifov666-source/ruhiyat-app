...
export type StepUpMeta = { title?: string; description?: string }

type Resolver<T> = { resolve: (v: T) => void; reject: (e: unknown) => void; retry: () => Promise<T> }
    ...
    try {
      ...
      try {
        ...
        if (isStepUpRequiredError(retryErr)) {
          ...
          if (retryErr instanceof ApiHttpError && retryErr.statusCode === 403) {
      ...
      if (err instanceof VerifyPasswordError) {
        toast.error("Parol noto'g'ri yoki tekshiruv bajarilmadi", { description: err.message })
        return
      }
